...
namespace NetTopologySuite.IO
{
    ...
    public class ShapefileDataWriter
    {
        ...
        {
            if (!IoManager.File.FileExists(dbfFile))
                throw new FileNotFoundException(dbfFile + " not found");
            DbaseFileHeader header = new DbaseFileHeader();
            header.ReadHeader(new BinaryReader(IoManager.File.CreateFileStream(dbfFile, FileMode.Open, FileAccess.Read, FileShare.Read)), dbfFile);
            return header;
        }
