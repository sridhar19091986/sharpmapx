    ...
    public class DbaseFileHeader
    {
        ...
        public void AddColumn(string fieldName, char fieldType, int fieldLength, int decimalCount)
        {
            ...
            if ((fieldType == 'C') || (fieldType == 'c'))
            {
                ...
                if (fieldLength > 254) Debug.WriteLine("Field Length for " + fieldName + " set to " + fieldLength + " Which is longer than 254, not consistent with dbase III");
            }
        ...
        private Encoding DetectEncodingFromMark(int lcid, string cpgFileName)
        {
            ...
            else
            {
                ...
                {
                    cpgFileName = IoManager.File.ChangeExtension(cpgFileName, "cpg");
                    if (!IoManager.File.FileExists(cpgFileName))
                        cpgFileName = IoManager.File.ChangeExtension(cpgFileName, "cst");
                    if (IoManager.File.FileExists(cpgFileName))
                    {
                        var encoding = IoManager.File.ReadAllText(cpgFileName).Trim();
                        try
                        {
