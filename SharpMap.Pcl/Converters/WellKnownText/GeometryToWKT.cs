...
namespace SharpMap.Converters.WellKnownText
{
    ...
    public class GeometryToWKT
    {
        ...
        {
            writer.Write(WriteNumber(coordinate.X) + " " + WriteNumber(coordinate.X));
        }
#else
        private static void AppendCoordinate(Coordinate coordinate, StringWriter writer)
        {
            for (uint i = 0; i < coordinate.NumPoints; i++)
                writer.Write(WriteNumber(coordinate[i]) + (i < coordinate.NumPoints - 1 ? " " : ""));
        }
#endif
