    ...
    public class LayerGroup : Layer, IDisposable
    {
        ...
        public override IEnvelope Envelope
        {
            get
            {
                if (Layers.Count == 0)
                    return null;
                IEnvelope bbox = Layers[0].Envelope;
                for (int i = 1; i < Layers.Count; i++)
                    bbox = bbox.Union(Layers[i].Envelope);
                return bbox;
            }
        }
...
#if PCL
                if (String.Equals(_Layers[i].LayerName, name, StringComparison.CurrentCultureIgnoreCase))
                    return _Layers[i];
#else
        ...
        //!!!}
         ...
         public override IFeatures GetFeaturesInView(Envelope box, double resolution)
        {
            foreach (Layer layer in Layers)
            {
