...
namespace NetTopologySuite.IO
{
	...
	public class ShapefileWriter : IDisposable
	{
        ...
        {
            var folder = IoManager.File.GetDirectoryName(filename) ?? ".";
            var file = IoManager.File.GetFileNameWithoutExtension(filename);
            if (string.IsNullOrEmpty(file))
                throw new ArgumentException(string.Format("Filename '{0}' is not valid", filename), "filename");
            filename = IoManager.File.PathCombine(folder, file);

            _shpStream = IoManager.File.CreateFileStream(filename + ".shp", FileMode.Create);
            _shxStream = IoManager.File.CreateFileStream(filename + ".shx", FileMode.Create);
        ...
        {
            filename = IoManager.File.ChangeExtension(filename, "dbf");
